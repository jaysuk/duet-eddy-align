 ...
 *
 *   node scripts/release.mjs 0.16.0          # bump + commit + tag (you push when ready)
 *   node scripts/release.mjs 0.16.0 --push   # …and push the commit + tag (triggers the release)
 *   npm run release -- 0.16.0 [--push]
 *
 * Ported from duet-tool-align's scripts/release.mjs.
 */
...
if (!version || !/^\d+\.\d+\.\d+(-[\w.]+)?$/.test(version)) {
	console.error("Usage: node scripts/release.mjs <version> [--push]   e.g. 0.16.0");
	process.exit(1);
}
const tag = `v${version}`;
...
git(["tag", "-a", tag, "-m", `DuetEddyAlign ${tag}`]);
