/**
 * Pure tool-offset maths. Ported verbatim from duet-tool-align (src/util/toolAlign.ts), which itself
 * copied it from Flexible-Layouts — keeping it byte-identical means an eventual merge, or a shared
 * extraction, stays a no-op instead of a redesign. Only the doc comment below is duplicated on
 * purpose; the logic must not drift from the source.
 *
 ...
 * on BOTH the reference and the tool contribute.
 *
 ...
 * land the carriage at X=95, so 95 = 100 − offset ⟹ offset = +5 = M_ref − M_t, not M_t − M_ref.
 */
